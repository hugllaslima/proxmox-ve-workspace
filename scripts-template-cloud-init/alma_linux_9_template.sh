...
#   AlmaLinux 9 utilizando Cloud-Init no Proxmox VE.
#
# Funcionalidades:
#   - Coleta de dados (ID, Nome, Storage, Tamanho do disco).
#   - Download e verificação da imagem AlmaLinux 9 Cloud-Init.
...
#   - Conexão com a internet para baixar a imagem cloud-init do AlmaLinux.
#   - Espaço suficiente no storage de destino para o disco da VM.
#
# Como usar:
#   1. chmod +x almalinux_9_template.sh
#   2. ./almalinux_9_template.sh
...
#
# Notas Importantes:
#   - A imagem baixada ('AlmaLinux-9-GenericCloud-latest.x86_64.qcow2') será armazenada em
...
echo -e "${GREEN}=== Proxmox Cloud-Init Template Creator (Alma Linux 9) ===${NC}"
...
# 2. Pergunta para obter o Nome da VM
read -p "Digite o Nome para o Template (ex: alma-linux-9-template): " VM_NAME

# 3. Lista Storages disponíveis e pergunta para obter o storage de onde alocar o disco
echo -e "\nStorages disponíveis:"
pvesm status | awk 'NR>1 {print "- " $1}'
read -p "Em qual storage o disco será alocado? (ex: local): " STORAGE

# 4. Pergunta para obter o tamanho do disco
...
# URL da imagem Cloud-Init do AlmaLinux 9
IMAGE_URL="https://repo.almalinux.org/almalinux/9/cloud/x86_64/images/$IMAGE_NAME"

echo -e "\n${GREEN}[1/6] Baixando/Verificando imagem AlmaLinux 9...${NC}"
mkdir -p $IMAGE_DIR
...
if [[ "$CONFIRM" == "s" || "$CONFIRM" == "S" ]]; then
    ...
    echo -e "   - ${GREEN}User${NC}: almalinux (O usuário padrão do AlmaLinux Cloud Image)"
