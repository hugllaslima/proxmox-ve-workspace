#!/bin/bash
# ==============================================================================
# SCRIPT PARA CRIAÇÃO DE TEMPLATE CLOUD-INIT CENTOS STREAM 9 NO PROXMOX VE
# ==============================================================================
#
# Script: centos_stream_9_template.sh
#
# Descrição:
#   Este script automatiza a criação de um template de máquina virtual (VM)
#   CentOS Stream 9 utilizando Cloud-Init no Proxmox VE.
#
# Funcionalidades:
#   - Coleta de dados (ID, Nome, Storage, Tamanho do disco).
#   - Download e verificação da imagem CentOS Stream 9 Cloud-Init.
...
#   - Conexão com a internet para baixar a imagem cloud-init do CentOS Stream.
#   - Espaço suficiente no storage de destino para o disco da VM.
#
# Como usar:
#   1. chmod +x centos_stream_9_template.sh
...
#
# Notas Importantes:
#   - A imagem baixada ('CentOS-Stream-GenericCloud-9-latest.x86_64.qcow2') será armazenada em
...
echo -e "${GREEN}=== Proxmox Cloud-Init Template Creator (CentOS Stream 9) ===${NC}"

# 1. Pergunta o ID da VM
while true; do
    read -p "Digite o ID para a nova VM Template (ex: 9005): " VM_ID
...
read -p "Digite o Nome para o Template (ex: centos-stream-9-template): " VM_NAME
...
IMAGE_NAME="CentOS-Stream-GenericCloud-9-latest.x86_64.qcow2"

# URL da imagem Cloud-Init do CentOS Stream 9
IMAGE_URL="https://cloud.centos.org/centos/9-stream/x86_64/images/$IMAGE_NAME"

echo -e "\n${GREEN}[1/6] Baixando/Verificando imagem CentOS Stream 9...${NC}"
mkdir -p $IMAGE_DIR
...
if [[ "$CONFIRM" == "s" || "$CONFIRM" == "S" ]]; then
    ...
    echo -e "   - ${GREEN}User${NC}: centos (O usuário padrão do CentOS Cloud Image)"
