#!/bin/bash
# ==============================================================================
# SCRIPT PARA CRIAÇÃO DE TEMPLATE CLOUD-INIT DEBIAN 10 NO PROXMOX VE
# ==============================================================================
#
# Script: debian_10_template.sh
#
# Descrição:
#   Este script automatiza a criação de um template de máquina virtual (VM)
#   Debian 10 (Buster) utilizando Cloud-Init no Proxmox VE.
#
# Funcionalidades:
#   - Coleta de dados (ID, Nome, Storage, Tamanho do disco).
#   - Download e verificação da imagem Debian 10 Cloud-Init.
...
#   - Conexão com a internet para baixar a imagem cloud-init do Debian.
#   - Espaço suficiente no storage de destino para o disco da VM.
#
# Como usar:
#   1. chmod +x debian_10_template.sh
...
#
# Notas Importantes:
#   - A imagem baixada ('debian-10-generic-amd64.qcow2') será armazenada em
...
echo -e "${GREEN}=== Proxmox Cloud-Init Template Creator (Debian 10) ===${NC}"
...
read -p "Digite o Nome para o Template (ex: debian-10-template): " VM_NAME
...
IMAGE_NAME="debian-10-generic-amd64.qcow2"

# URL da imagem Cloud-Init do Debian 10 (Buster).
IMAGE_URL="https://cloud.debian.org/images/cloud/buster/latest/debian-10-generic-amd64.qcow2"

echo -e "\n${GREEN}[1/6] Baixando/Verificando imagem Debian 10...${NC}"
mkdir -p $IMAGE_DIR
...
if [[ "$CONFIRM" == "s" || "$CONFIRM" == "S" ]]; then
    ...
    echo -e "   - ${GREEN}User${NC}: debian (O usuário padrão do Debian Cloud Image)"
