#!/bin/bash
# ==============================================================================
# SCRIPT PARA CRIAÇÃO DE TEMPLATE CLOUD-INIT ALPINE LINUX NO PROXMOX VE
# ==============================================================================
#
# Script: alpine_linux_template.sh
#
# Descrição:
#   Este script automatiza a criação de um template de máquina virtual (VM)
#   Alpine Linux utilizando Cloud-Init no Proxmox VE.
#   Configuração leve: 1 Core e 1GB de RAM por padrão.
#
# Funcionalidades:
#   - Coleta de dados (ID, Nome, Storage, Tamanho do disco).
#   - Download e verificação da imagem Alpine Linux Cloud-Init.
#   - Criação da estrutura base da VM (1 CPU, 1GB RAM).
...
#   - Conexão com a internet para baixar a imagem cloud-init do Alpine Linux.
#   - Espaço suficiente no storage de destino para o disco da VM.
#
# Como usar:
#   1. chmod +x alpine_linux_template.sh
...
#
# Notas Importantes:
#   - A imagem baixada ('nocloud_alpine-3.21.2-x86_64-bios-cloudinit-r0.qcow2') será armazenada em
...
echo -e "${GREEN}=== Proxmox Cloud-Init Template Creator (Alpine Linux) ===${NC}"

# 1. Pergunta o ID da VM
while true; do
    read -p "Digite o ID para a nova VM Template (ex: 9009): " VM_ID
...
read -p "Digite o Nome para o Template (ex: alpine-linux-template): " VM_NAME
...
read -p "Qual o tamanho final do disco em GB? (ex: 5): " DISK_SIZE

# --- Início do Processo ---

IMAGE_DIR="/var/lib/vz/template/iso"
IMAGE_NAME="nocloud_alpine-3.21.2-x86_64-bios-cloudinit-r0.qcow2"

# URL da imagem Cloud-Init do Alpine Linux (versão 3.21.x)
IMAGE_URL="https://dl-cdn.alpinelinux.org/alpine/v3.21/releases/cloud/$IMAGE_NAME"

echo -e "\n${GREEN}[1/6] Baixando/Verificando imagem Alpine Linux...${NC}"
mkdir -p $IMAGE_DIR
...
echo -e "${GREEN}[2/6] Criando a estrutura da VM (1 CPU, 1GB RAM)...${NC}"
qm create $VM_ID --name "$VM_NAME" --memory 1024 --cores 1 --net0 virtio,bridge=vmbr0 --ostype l26 --agent 1 || exit 1
...
if [[ "$CONFIRM" == "s" || "$CONFIRM" == "S" ]]; then
    ...
    echo -e "   - ${GREEN}User${NC}: alpine (O usuário padrão adotado em provedores de nuvem para a Alpine Linux)"
    ...
    echo -e "4. Ligue a VM e acesse o console para rodar os comandos (Alpine usa apk):"
    echo -e "   ${GREEN}$ sudo apk update && sudo apk add qemu-guest-agent${NC}"
    echo -e "   ${GREEN}$ sudo rc-update add qemu-guest-agent${NC}"
    ...
    echo -e "6. Desligue a VM (poweroff)."
