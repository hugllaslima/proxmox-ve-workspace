#!/bin/bash
# ==============================================================================
# SCRIPT PARA CRIAÇÃO DE TEMPLATE CLOUD-INIT ROCKY LINUX 9 NO PROXMOX VE
# ==============================================================================
#
# Script: rocky_linux_9_template.sh
#
# Descrição:
#   Este script automatiza a criação de um template de máquina virtual (VM)
#   Rocky Linux 9 utilizando Cloud-Init no Proxmox VE.
#
# Funcionalidades:
#   - Coleta de dados (ID, Nome, Storage, Tamanho do disco).
#   - Download e verificação da imagem Rocky Linux 9 Cloud-Init.
...
#   - Conexão com a internet para baixar a imagem cloud-init do Rocky Linux.
#   - Espaço suficiente no storage de destino para o disco da VM.
#
# Como usar:
#   1. chmod +x rocky_linux_9_template.sh
#   2. ./rocky_linux_9_template.sh
...
#
# Notas Importantes:
#   - A imagem baixada ('Rocky-9-GenericCloud.latest.x86_64.qcow2') será armazenada em
...
echo -e "${GREEN}=== Proxmox Cloud-Init Template Creator (Rocky Linux 9) ===${NC}"

# 1. Pergunta o ID da VM
while true; do
    read -p "Digite o ID para a nova VM Template (ex: 9004): " VM_ID
...
read -p "Digite o Nome para o Template (ex: rocky-linux-9-template): " VM_NAME
...
# URL da imagem Cloud-Init do Rocky Linux 9
IMAGE_URL="https://download.rockylinux.org/pub/rocky/9/images/x86_64/$IMAGE_NAME"

echo -e "\n${GREEN}[1/6] Baixando/Verificando imagem Rocky Linux 9...${NC}"
mkdir -p $IMAGE_DIR
...
if [[ "$CONFIRM" == "s" || "$CONFIRM" == "S" ]]; then
    ...
    echo -e "   - ${GREEN}User${NC}: rocky (O usuário padrão do Rocky Linux Cloud Image)"
