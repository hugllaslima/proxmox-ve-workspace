...
################################################################################
# Script de Atualização Proxmox VE 6.x → 7.x (Sem Subscrição / No-Subscription)
# Autor: SysAdmin / Hugllas Lima
# Data: 2026-09-12
# Versão: 1.0
# Descrição: Upgrade seguro e interativo de Proxmox VE 6.4 (Debian 10 Buster)
#             para Proxmox VE 7.4 (Debian 11 Bullseye) sem repositório Enterprise.
#
# Referência oficial:
...
SCRIPT_VERSION="1.0"
LOG_FILE="/var/log/proxmox-upgrade-v6-to-v7-$(date +%Y%m%d-%H%M%S).log"
...
FROM_DEBIAN="buster"     # Debian 10
TO_DEBIAN="bullseye"     # Debian 11
...
check_prerequisites() {
    ...
    if [ "$DEBIAN_CODENAME" != "$FROM_DEBIAN" ]; then
        print_color $YELLOW "⚠ ATENÇÃO: Este host está executando o Debian '$DEBIAN_CODENAME' (esperado: '$FROM_DEBIAN' para Proxmox 6.x)."
...
################################################################################
# MIGRAÇÃO DE REPOSITÓRIOS PARA BULLSEYE + PROXMOX 7 (NO-SUBSCRIPTION)
################################################################################

switch_repositories_to_bullseye() {
    print_line
    print_color $CYAN "🔄 Configurando Repositórios Oficiais do Proxmox VE 7 (Debian Bullseye)..."
    print_line
    ...
    print_color $BLUE "▶ Escrevendo fontes limpas do Proxmox 7 em /etc/apt/sources.list..."
    cat << 'EOF' > /etc/apt/sources.list
# Repositórios Oficiais Debian 11 (Bullseye)
deb http://deb.debian.org/debian bullseye main contrib non-free
deb http://deb.debian.org/debian bullseye-updates main contrib non-free

# Repositório Proxmox VE 7 No-Subscription
deb http://download.proxmox.com/debian/pve bullseye pve-no-subscription

# Atualizações de Segurança Debian 11
deb http://security.debian.org/debian-security bullseye-security main contrib non-free
EOF

    log "INFO" "Arquivo /etc/apt/sources.list configurado com as fontes oficiais do Proxmox 7"
...
################################################################################
# VERIFICAÇÃO PRÉVIA — pve6to7
################################################################################

run_pve6to7_check() {
    print_line
    print_color $CYAN "🔍 Executando Verificação Prévia de Compatibilidade (pve6to7)..."
    print_line

    if ! command -v pve6to7 &> /dev/null; then
        print_color $YELLOW "ℹ️ O comando 'pve6to7' não está presente. Tentando instalar via apt..."
        apt-get update -qq >> "$LOG_FILE" 2>&1 || true
        apt-get install -y pve6to7 >> "$LOG_FILE" 2>&1 || true
    fi

    if command -v pve6to7 &> /dev/null; then
        print_color $BLUE "▶ Executando checklist oficial 'pve6to7 --full'..."
        pve6to7 --full | tee -a "$LOG_FILE"
        local status=${PIPESTATUS[0]}
        if [ $status -eq 0 ]; then
            print_color $GREEN "✓ Checklist pve6to7 aprovado sem erros críticos."
        else
            print_color $YELLOW "⚠ O pve6to7 reportou avisos ou recomendações (código $status)."
            if ! confirm "Deseja prosseguir com o upgrade mesmo com os avisos acima?"; then
                error_exit "Migração cancelada pelo usuário para revisão do pve6to7."
            fi
        fi
    else
        print_color $YELLOW "⚠ Não foi possível instalar o pve6to7. Prosseguindo sem verificação oficial..."
    fi
}

################################################################################
# UPGRADE MAIOR: PROXMOX VE 6.4 → 7.4
################################################################################

major_upgrade_6_to_7() {
    print_line
    print_color $RED "🚀 UPGRADE MAIOR: PROXMOX VE 6.4 → 7.4 (MIGRAÇÃO DE SISTEMA)"
    print_line
    cat << 'EOF'
Este procedimento irá realizar a MIGRAÇÃO COMPLETA:
  • Validação de pré-requisitos e teste de compatibilidade (pve6to7)
  • Backup completo das configurações (/etc/pve, cluster SQLite, redes, VMs)
  • Garantia de que a base 6.4 atual está totalmente atualizada
  • Migração dos repositórios: Debian 10 (Buster) → Debian 11 (Bullseye)
  • Instalação dos pacotes do Proxmox VE 7.4 e novo Kernel Linux
...
EOF
    ...
    if ! confirm "Tem certeza que deseja iniciar o Upgrade para Proxmox VE 7.4 agora?"; then
        log "INFO" "Upgrade maior cancelado pelo usuário"
        print_color $YELLOW "Operação cancelada."
        return 0
    fi

    log "INFO" "Iniciando processo de Upgrade Proxmox 6.4 para 7.4"

    check_prerequisites
    run_pve6to7_check
    create_backup

    # Etapa 1: Atualiza os pacotes da versão atual (6.x) antes de migrar
    print_color $CYAN "▶ Etapa 1/4: Atualizando todos os pacotes da versão 6.x atual..."
    run_with_spinner "Atualizando base 6.x antes da migração (apt update)" "apt-get update" true
    run_with_spinner "Aplicando patches atuais do Proxmox 6 (apt dist-upgrade)" \
        "DEBIAN_FRONTEND=noninteractive apt-get dist-upgrade -y" true

    # Etapa 2: Migra repositórios para Bullseye
    print_color $CYAN "▶ Etapa 2/4: Migrando repositórios para Debian 11 Bullseye + Proxmox 7..."
    switch_repositories_to_bullseye

    # Etapa 3: Major Upgrade
    echo ""
    print_color $YELLOW "╔════════════════════════════════════════════════════════════════════════════╗"
    print_color $YELLOW "║ 🚀 ETAPA 3/4: INSTALANDO PROXMOX VE 7.4 E KERNEL                         ║"
    print_color $YELLOW "║    • Baixando e instalando pacotes do Proxmox 7 e Debian 11 (Bullseye).   ║"
    ...
    run_with_spinner "Baixando catálogos do Debian 11 Bullseye e Proxmox 7" "apt-get update" true
    run_with_spinner "Instalando Proxmox VE 7.4 e Kernel (apt-get dist-upgrade)" \
        "DEBIAN_FRONTEND=noninteractive apt-get dist-upgrade -y" true
    ...
    print_color $CYAN "▶ Etapa 4/4: Verificando integridade do Proxmox VE 7..."
    post_update_check
    print_recommendations

    echo ""
    print_color $GREEN "🎉 O upgrade para o Proxmox VE 7.4 foi concluído com sucesso!"
    print_color $YELLOW "🔔 Para inicializar com o novo kernel do Proxmox 7, o servidor deve ser reiniciado."
    echo ""

    if confirm "Deseja reiniciar o servidor Proxmox agora para carregar a versão 7.4?"; then
        log "INFO" "Usuário autorizou reboot pós-upgrade para Proxmox 7"
        print_color $RED "⚠️  O servidor será REINICIADO em 5 segundos..."
        print_color $YELLOW "Pressione Ctrl+C para cancelar imediatamente."
        sleep 5
        log "INFO" "Executando comando reboot"
        ...
        print_color $GREEN "✓ Reinicialização adiada. Para concluir e ativar o Proxmox 7.4, execute quando for conveniente: reboot"
    fi
}
...
show_banner() {
    ...
    box_center "🚀 Upgrade Proxmox VE 6 → 7 (No-Subscription)"
    print_color $BLUE "$empty"
...
show_menu() {
    ...
    print_line
    echo "1) 🚀 Realizar Upgrade Maior: Proxmox VE 6.4 → 7.4 (Debian Bullseye)"
    echo "2) 🔄 Atualização Regular de Pacotes (Manter versão atual 6.x)"
    echo "3) 🔍 Executar Verificação Prévia de Compatibilidade (pve6to7)"
    echo "4) 💾 Apenas criar backup das configurações"
    echo "5) ⚙️  Apenas configurar repositórios No-Subscription (Bullseye)"
...
main() {
    ...
    log "INFO" "═══════════════════════════════════════════════════════════"
    log "INFO" "Assistente de Upgrade Proxmox VE 6 → 7 iniciado"
    ...
    while true; do
        ...
        case "$option" in
            ...
            5)
                check_prerequisites
                switch_repositories_to_bullseye
                print_color $GREEN "✓ Repositórios No-Subscription (Bullseye) configurados com sucesso."
                ;;
