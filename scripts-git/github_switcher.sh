...
# ==============================================================================
# SCRIPT PARA ALTERNAR USUÁRIOS GIT/GITHUB EM UM REPOSITÓRIO LOCAL (GENÉRICO)
# ==============================================================================
# Este script permite que você configure e alterne facilmente entre múltiplas
# contas Git/GitHub (pessoal, trabalho, etc.) em seus repositórios locais.
...
# local (~/.git_switcher_accounts.conf) para uso futuro.
#
...
#    conta GitHub que deseja gerenciar.
...
echo "----------------------------------------------------"
echo "  Bem-vindo ao Alternador de Usuários Git/GitHub!   "
echo "----------------------------------------------------"
...
display_accounts() {
    ...
    for account_entry in "${ACCOUNTS_ARRAY[@]}"; do
        IFS='|' read -r name email ssh_host github_user pubkey_path <<< "$account_entry"
        echo "  $((i+1))) $name ($email)"
        ((i++))
    done
...
add_account_interactively() {
    echo "----------------------------------------------------"
    echo "  Adicionar Nova Conta Git/GitHub                   "
    echo "----------------------------------------------------"
    local new_name new_email new_ssh_host new_github_user new_pubkey_path
    ...
    read -p "Host SSH (ex: github.com-pessoal, github.com-trabalho ou github.com-seu_usuario): " new_ssh_host
    # --- FIM DA ALTERAÇÃO ---
    read -p "Usuário/Organização GitHub (ex: seu-username ou nome-da-org): " new_github_user
    ...
    ACCOUNTS_ARRAY+=("$new_name|$new_email|$new_ssh_host|$new_github_user|$new_pubkey_path")
...
edit_account_interactively() {
    ...
    IFS='|' read -r current_name current_email current_ssh_host current_github_user current_pubkey_path <<< "$current_entry"
    ...
    read -p "Novo Host SSH (atual: $current_ssh_host, ex: github.com-seu_usuario): " new_ssh_host_input
    # --- FIM DA ALTERAÇÃO ---
    read -p "Novo Usuário/Organização GitHub (atual: $current_github_user): " new_github_user_input
    ...
    local final_github_user=${new_github_user_input:-$current_github_user}
    ...
    ACCOUNTS_ARRAY[$index]="$final_name|$final_email|$final_ssh_host|$final_github_user|$final_pubkey_path"
    save_accounts
...
check_ssh_config() {
    ...
    # 2. Perguntar se a chave já está no GitHub
    read -p "A chave pública para '$account_name' (arquivo: $(basename "$pubkey_path")) já está configurada no GitHub? (s/n): " ssh_configured_choice
    if [[ "$ssh_configured_choice" =~ ^[Ss]$ ]]; then
        echo "Realizando teste de conexão SSH para '$account_name'..."
        # Tentar conectar ao GitHub usando o host SSH configurado
        ...
        if ssh -T git@"$ssh_host" 2>&1 | grep -q "successfully authenticated"; then
            ...
            echo "  - Se a chave pública está adicionada corretamente na sua conta GitHub."
            echo "  - Tente 'eval \"\$(ssh-agent -s)\"' e 'ssh-add \"$private_key_path_relative\"' se você usa passphrase."
            exit 1
        fi
    else
        echo "Por favor, adicione a chave pública ao GitHub para a conta '$account_name'."
        echo "1. Copie o conteúdo da sua chave pública:"
        echo "   cat $pubkey_path"
        echo "2. Acesse as configurações SSH do GitHub para a conta '$github_user':"
        echo "   https://github.com/settings/keys"
        echo "3. Clique em 'New SSH key', cole o conteúdo e salve."
        ...
        1) # Configurar repositório atual
            ...
            while true; do
                ...
                if [[ "$select_account_choice" =~ ^[Qq]$ ]]; then
                    ...
                    IFS='|' read -r SELECTED_NAME SELECTED_EMAIL SELECTED_SSH_HOST SELECTED_GITHUB_USER SELECTED_PUBKEY_PATH <<< "$selected_account_entry"
                    ...
                    check_ssh_config "$SELECTED_NAME" "$SELECTED_SSH_HOST" "$SELECTED_PUBKEY_PATH" "$SELECTED_GITHUB_USER"
                    ...
                    if [ -z "$CURRENT_REMOTE_URL" ]; then
                        ...
                        echo "Por favor, configure o remote manualmente se necessário (ex: git remote add origin git@$SELECTED_SSH_HOST:$SELECTED_GITHUB_USER/nome-do-repositorio.git)"
                    else
                        REPO_NAME=$(basename "$CURRENT_REMOTE_URL")
                        NEW_REMOTE_URL="git@$SELECTED_SSH_HOST:$SELECTED_GITHUB_USER/$REPO_NAME"
                    ...
                    echo ""
                    echo "Ou também pode verificar o usuário conectado com:"
